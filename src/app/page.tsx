...
//   const images = ["/logo3.jpeg", "/logo4.jpeg", "/logo5.jpeg", "/logo6.jpeg", "/logo7.jpeg"];
...
/>
  ...
  <div className="relative w-full flex justify-center overflow-hidden px-4">
    <div className="slider-container w-full max-w-6xl">
      <div className="slider-track flex">
        {[...images, ...images].map((img, index) => (
          <div key={index} className="slide flex-shrink-0 w-[250px] h-[100px] flex items-center justify-center">
            <img src={img} alt="Partner Logo" className="object-contain rounded-lg shadow-lg w-full h-full" />
          </div>
        ))}
      </div>
    </div>
  ...
  <style jsx>{`
    .slide {
      margin: 0 16px; /* Consistent spacing between slides */
    }
    .slider-container {
      overflow: hidden;
    }
    .slider-track {
      display: flex;
      animation: scroll 10s linear infinite; /* Faster movement */
    }
    @keyframes scroll {
      0% {
        transform: translateX(0);
